...
class CASIAWebFace_Doppelganger_loader(Dataset):
    def __init__(self, root_dir, transform=None, other_dataset=None):
        super(CASIAWebFace_Doppelganger_loader, self).__init__()
        ...
        self.samples_list = self.make_samples_list_with_labels_doppelganger(self.path_files, self.subjs_list, self.subjs_dict, self.races_dict, self.genders_dict)
    ...
    def get_subj_race_gender_dicts(self, path_files):
        ...
        subjs_list = ud.natural_sort(list(set(subjs_list)))
    ...
    def make_samples_list_with_labels_doppelganger(self, path_files, subjs_list, subjs_dict, races_dict, genders_dict):
        ...
        for i, (path_file, dataset_name) in enumerate(path_files):        # '/datasets2/frcsyn_wacv2024/datasets/real/1_CASIA-WebFace/imgs_crops_112x112/0/1.png'
            ...
            doppel_idx = -1
            if "_ArcFace_" in path_file:
                doppel_idx = int(path_file.split('/')[-2].split('_')[0])

            subjs_dict_num_samples[subj] += 1
            samples_list[i] = (dataset_name, path_file, subj, doppel_idx, None, None)
    ...
    def replace_strings_labels_by_int_labels(self, samples_list, subjs_dict, races_dict, genders_dict):
        ...
        for i in range(len(final_samples_list)):
            ...
            dataset_name, path_file, subj, doppel_idx, race, gender = samples_list[i]
            ...
            final_samples_list[i] = [dataset_name, path_file, subj_idx, doppel_idx, race_idx, gender_idx]
    ...
    def __getitem__(self, index):
        ...
        # Bernardo
        dataset_name, img_path, subj_idx, doppel_idx, race_idx, gender_idx = self.final_samples_list[index]
        ...
        return (rgb_data, subj_idx, doppel_idx, race_idx, gender_idx)
