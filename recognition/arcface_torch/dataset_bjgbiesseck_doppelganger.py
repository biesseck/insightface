...
def merge_dataloaders_doppelganger(dataloader1=[], dataloader2=[]):
    ...
    doppelganger_dict_dataloader2 = {}
    for i, sample_dataset2 in enumerate(dataloader2.final_samples_list):
        #                         0           1          2          3          4         5
        # sample_dataset2 = [dataset_name, img_path, subj_idx, doppel_idx, race_idx, gender_idx]
        if not sample_dataset2[3] in doppelganger_dict_dataloader2:
            doppelganger_dict_dataloader2[sample_dataset2[3]] = sample_dataset2[2]
    for i, sample_dataset1 in enumerate(dataloader1.final_samples_list):
        #                         0           1          2         3         4
        # sample_dataset1 = [dataset_name, img_path, subj_idx, race_idx, gender_idx]
        if sample_dataset1[2] in doppelganger_dict_dataloader2:
            sample_dataset1[3] = doppelganger_dict_dataloader2[sample_dataset1[2]]
...
def get_dataloader_doppelganger(
    root_dir,
    local_rank,
    batch_size,
    dali = False,
    ...
    if type(root_dir) == list:
        ...
        # Image Folder
            ...
            if 'CASIA-WebFace'.lower() in root_dir.lower():
                from dataloaders.casiawebface_doppelganger_loader import CASIAWebFace_Doppelganger_loader
                print(f'Loading train dataset \'{root_dir}\' ...')
                train_set = CASIAWebFace_Doppelganger_loader(root_dir, transform)
            ...
            if hasattr(cfg, 'path_subjs_list_to_merge'):
                ...
                # Merge 2 dataloaders
                train_set = merge_dataloaders_doppelganger(train_set, train_set_from_json)

            if hasattr(cfg, 'path_other_dataset'):
                print(f'Loading other train dataset \'{cfg.path_other_dataset}\' ...')
                from dataloaders.casiawebface_doppelganger_loader import CASIAWebFace_Doppelganger_loader
                other_train_set = CASIAWebFace_Doppelganger_loader(cfg.path_other_dataset, transform)
