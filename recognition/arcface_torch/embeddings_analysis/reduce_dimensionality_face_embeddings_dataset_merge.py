# export CUDA_VISIBLE_DEVICES=0; python embeddings_analysis/reduce_dimensionality_face_embeddings_dataset_merge.py --input-path /hddevice/nobackup3/bjgbiesseck/datasets/face_recognition/CASIA-WebFace/imgs_crops_112x112_FACE_EMBEDDINGS --input-ext _id_feat.pt --merge-dataset-json /hddevice/nobackup3/bjgbiesseck/datasets/face_recognition/CASIA-WebFace/merge_with_dataset_MS-Celeb-1M-ms1m-retinaface-t1-imgs_FACE_EMBEDDINGS_sim-range=[0.5,0.69]/dict_paths_new_subjs_base_subjs.json --merge-dataset-path /hddevice/nobackup3/bjgbiesseck/datasets/face_recognition/MS-Celeb-1M/ms1m-retinaface-t1/imgs_FACE_EMBEDDINGS
...
def parse_args():
    ...
    parser.add_argument('--merge-dataset-json', type=str, default='')   # /hddevice/nobackup3/bjgbiesseck/datasets/face_recognition/CASIA-WebFace/merge_with_dataset_MS-Celeb-1M-ms1m-retinaface-t1-imgs_FACE_EMBEDDINGS_sim-range=[0.5,0.69]/dict_paths_new_subjs_base_subjs.json
    parser.add_argument('--merge-dataset-path', type=str, default='')   # /hddevice/nobackup3/bjgbiesseck/datasets/face_recognition/MS-Celeb-1M/ms1m-retinaface-t1/imgs_FACE_EMBEDDINGS
...
def update_path(current_path='', new_parent_path=''):
    new_parent_dir = new_parent_path.split('/')[-1]
    new_path = os.path.join(new_parent_path, current_path.split(new_parent_dir)[-1].strip('/'))
    return new_path
...
if __name__ == '__main__':
    ...
    if args.merge_dataset_json != '':
        dict_orig_paths_new_subjs = load_json(args.merge_dataset_json)
        dict_update_paths_new_subjs = {}
        for orig_path_new_subj in list(dict_orig_paths_new_subjs.keys()):
            orig_list_paths_base_subj = dict_orig_paths_new_subjs[orig_path_new_subj]
            
            update_path_new_subj = update_path(orig_path_new_subj, args.merge_dataset_path)
            update_paths_base_subjs = 
            dict_update_paths_new_subjs[update_path_new_subj] = update_paths_base_subjs
        
        # print('dict_paths_new_subjs:', dict_paths_new_subjs)
        # orig_paths_mean_embedds_new_subjs   = list(dict_paths_new_subjs.keys())
        # # print('orig_paths_mean_embedds_new_subjs:', orig_paths_mean_embedds_new_subjs)
        # update_paths_mean_embedds_new_subjs = [update_path(orig_path, args.merge_dataset_path) for orig_path in orig_paths_mean_embedds_new_subjs]
        # # print('update_paths_mean_embedds_new_subjs:', update_paths_mean_embedds_new_subjs)
        # sys.exit(0)

    elif args.num_classes > -1 and args.num_classes < max(embedds_classes_int):
